...
<<fig=TRUE>>=
library(DNAcopy)
plot(segres, plot.type="w", xmaploc=TRUE)
@
\end{center} 
...
\begin{center}
<<fig=TRUE>>=
plot(segres, plot.type="s", xmaploc=TRUE)
@
\end{center} 
...
segres <- toDNAcopyObj(
        segData     = res, 
        chrom       = rep(1, length(fastsegData)), 
        maploc      = 1:length(fastsegData), 
        genomdat    = as.matrix(fastsegData), 
        sampleNames = "sample1")
plot(segres, plot.type="w", xmaploc=TRUE)
@
\end{center} 
